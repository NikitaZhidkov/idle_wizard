...
/**
 * SPELL CASTER STRATEGY
 * Focus on magic - cast spells as soon as ready, prioritize spell damage buffs
 */
...
async function setupGame(page) {
  ...
  // Select Ravenclaw for +crit bonus (good for spell crits)
  ...
  log('SETUP', 'Game initialized with Ravenclaw (spell caster)');
}
...
async function getState(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      spellsReady: document.querySelectorAll('#spellsContainer .spell.ready').length,
      spellsTotal: document.querySelectorAll('#spellsContainer .spell').length
    };
  });
}

async function spellCasterStrategy(page) {
  // SPELL CASTER: Cast ALL ready spells in rapid succession
  ...
  for (let i = 0; i < count; i++) {
    try {
      ...
      const spellName = await spell.getAttribute('title') || await spell.textContent() || 'Unknown';
      await spell.click();
      castCount++;
      log('SPELL', `Cast: ${spellName}`);
      await page.waitForTimeout(50); // Rapid fire!
    } catch (e) { /* spell went on cooldown */ }
  }

  if (castCount > 0) {
    log('MAGIC', `Cast ${castCount} spells this round`);
  }
...
async function handleBuffSelection(page) {
  ...
  if (await buffChoice.isVisible({ timeout: 300 }).catch(() => false)) {
    // SPELL CASTER: Prioritize spell/magic buffs
    const spellBuff = page.locator('.buff-choice:has-text("Spell"), .buff-choice:has-text("Magic"), .buff-choice:has-text("Crit"), .buff-choice:has-text("Learn")').first();
    if (await spellBuff.isVisible().catch(() => false)) {
      await spellBuff.click();
      log('BUFF', 'Selected spell/magic buff');
    } else {
...
test('Spell caster strategy - magic focused gameplay', async ({ page }) => {
  const errors = [];
  ...
  log('TEST', 'Starting spell caster strategy');
  ...
  while (Date.now() - startTime < 240000) {
    ...
    if (state.floor >= 8) {
      log('TEST', `SUCCESS: Reached floor ${state.floor} with ${totalSpellsCast} spells cast`);
      break;
    }
    ...
    if (state.floor !== lastFloor) {
      log('FLOOR', `Floor ${state.floor}, HP: ${state.hp}, Spells: ${state.spellsReady}/${state.spellsTotal} ready`);
    ...
    // SPELL CASTER: Cast spells constantly
    const cast = await spellCasterStrategy(page);
    totalSpellsCast += cast;

    await page.waitForTimeout(300);
  }

  const finalState = await getState(page);
  log('TEST', `Final: Floor ${finalState.floor}, HP: ${finalState.hp}, Total spells cast: ${totalSpellsCast}`);

  expect(finalState.floor).toBeGreaterThanOrEqual(4);
  expect(totalSpellsCast).toBeGreaterThan(10); // Should have cast many spells
